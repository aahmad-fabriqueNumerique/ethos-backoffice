...
watchEffect(() => {
  if (songDetails) {
    console.log("🔄 Updating form from initialData:", songDetails);

    // Update interpretes array from initial data
    if (
      Array.isArray(songDetails.interpretes) &&
      songDetails.interpretes.length > 0
    ) {
      interpretesList.value = [...songDetails.interpretes];
    } else {
    ...
    <div class="w-full grid grid-cols-1 gap-4">
      ...
      <span class="flex flex-col gap-y-2">
        <Field v-slot="{ field, errorMessage }" name="description">
          <label for="description">{{ t("newSong.labels.description") }}</label>
          <Textarea
            id="description"
            fluid
            v-bind="field"
